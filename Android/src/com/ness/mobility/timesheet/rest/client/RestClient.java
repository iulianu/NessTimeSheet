...
public class RestClient {
        ...
        public Response execute(Request req) throws Exception {
        	addAllParams(req);
        	addAllHeaders(req);
        	String url = req.getRequestUri().toASCIIString();
                ...
                return  new Response(-3, null, "Method Unssuported:" + req.getMethod(), null);
        }

        private void addAllHeaders(Request req) {
			Map<String,List<String>> hrds = req.getHeaders();
			if (hrds != null) {
				for(Entry<String, List<String>> ent : hrds.entrySet()) {
				//TODO	addHeader(ent.getKey(), ent.getValue());
					
				}
			}
			
		}
		private void addAllParams(Request req) {
			Map<String,String> prms = req.getParams();
			if (prms != null) {
				for(Entry<String, String> ent : prms.entrySet()) {
					addParam(ent.getKey(), ent.getValue());
        ...
        private Response executeRequest(HttpUriRequest request, String url) {
                ...
                try {
                        ...
                        if (entity != null) {
                                ...
                                instream.close();
                                resp = new Response(responseCode, httpResponse.getAllHeaders(), null, response);
                        }
                        else
                        {
                        	resp = new Response(responseCode, httpResponse.getAllHeaders(), message, null);
                        }
                        ...
                        e.printStackTrace();
                        return new Response(-1, null, "Client Protocol Problem", null);
                } catch (IOException e) {
                        client.getConnectionManager().shutdown();
                        e.printStackTrace();
                        return new Response(-2, null, "IO Problem", null);
                }
        }
